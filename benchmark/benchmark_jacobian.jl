...
function finite_difference_jacobian(x0, parameters; h = 1e-8)
    f_eval = zeros(6)
    HighFidelityEphemerisModel.eom_Nbody!(f_eval, x0, parameters, 0.0)
    ...
    for i = 1:6
        ...
        x0_copy[i] += h
        _f_eval = zeros(6)
        HighFidelityEphemerisModel.eom_Nbody!(_f_eval, x0_copy, parameters, 0.0)
...
benchmark_jacobian = function(;verbose::Bool = false)
    ...
    # evaluate Jacobian
    jac_numerical = finite_difference_jacobian(x0, parameters)
    jac_generic = HighFidelityEphemerisModel.eom_jacobian_fd(
        HighFidelityEphemerisModel.eom_Nbody, x0, 0.0, parameters, 0.0
    )
    ...
    print_matrix(jac_generic)
    println()
    @test isapprox(jac_numerical, jac_generic, atol=1e-6)
