...
test_eom_stm_Nbody_Interp_with_drag = function()
    naif_ids = ["399"]
    GMs = [bodvrd(ID, "GM", 1)[1] for ID in naif_ids]
    naif_frame = "J2000"
    abcorr = "NONE"
    ...
    dx = HighFidelityEphemerisModel.eom_Nbody_Interp(x0, parameters, t)
    dx_stm = similar(x0_stm)
    ...
    HighFidelityEphemerisModel.eom_stm_Nbody_Interp_fd!(dx_stm_fd, x0_stm, parameters, t)
    @test maximum(abs.(dx_stm[7:42] - dx_stm_fd[7:42])) < 1e-6

    jac = HighFidelityEphemerisModel.dfdx_Nbody_Interp(x0, 0.0, parameters, t)
    jac_fd = HighFidelityEphemerisModel.eom_jacobian_fd(
        HighFidelityEphemerisModel.eom_Nbody_Interp, x0, 0.0, parameters, t
    )
...
test_eom_stm_Nbody_Interp_with_drag()
