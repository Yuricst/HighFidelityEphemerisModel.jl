...
function eom_stm_Nbody_Interp!(dx_stm, x_stm, params, t)
    if params.include_drag
        ...
        A = eom_jacobian_central_fd(eom_Nbody_Interp, x_stm[1:6], 0.0, params, t)
...
function dfdx_Nbody_Interp(x, u, params, t)
    if params.include_drag
        A = eom_jacobian_central_fd(eom_Nbody_Interp, x[1:6], u, params, t)
        A[1:3,4:6] .= I(3)
        return A
    end
...
function eom_stm_Nbody_Interp_fd!(dx_stm, x_stm, params, t)
    ...
    A = params.include_drag ?
        eom_jacobian_central_fd(eom_Nbody_Interp, x_stm[1:6], 0.0, params, t) :
        eom_jacobian_fd(eom_Nbody_Interp, x_stm[1:6], 0.0, params, t)
    A[1:3,4:6] .= I(3)   # force identity for linear map
    dx_stm[7:42] = reshape((A * reshape(x_stm[7:42],6,6)), 36)
    return nothing
end
